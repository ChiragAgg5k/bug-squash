...
const Navbar = () => {
	...
	return (
		<nav className="navbar absolute flex w-full items-center justify-between bg-base-100 px-4 py-3 text-white">
			{/* Logo */}
			<Image
				src="/logo.png"
				onClick={() => {
			...
			<ul className="mr-2 hidden flex-1 list-none items-center justify-end sm:flex">
				{navLinks.map((nav, index) => (
					<li
						key={nav.id}
						className={`cursor-pointer text-white ${index === navLinks.length - 1 ? "mr-0" : "mr-10"}`}
					>
						<Link href={`/${nav.id}`} className="group transition duration-300 hover:cursor-pointer">
							{nav.title}
							<span className="block h-[1px] max-w-0 bg-white transition-all duration-500 group-hover:max-w-full"></span>
						</Link>
					</li>
				))}
			</ul>
